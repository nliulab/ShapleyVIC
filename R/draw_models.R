...
draw_models <- function(coef_optim, coef_optim_var, x, y,
  ...
  df <- as.data.frame(cbind(
    id = 1:M,
    coef_mat,
    perf_metric = as.numeric(loss_vec) / loss_optim,
    eligible = (as.numeric(loss_vec) / loss_optim) < (1 + epsilon)
  ))
  p1 <- ggplot(data = df, aes_string(x = "perf_metric")) +
    geom_histogram() +
  ...
  if (is.null(n_final)) {
    ...
    df <- df[df$select == 1, setdiff(names(df), c("id", "eligible", "select"))]
  }
}
