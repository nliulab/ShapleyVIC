...
def mark_eligibility(coef_df, coef_optim, loss_func, 
                        criterion = 'loss', epsilon = 0.05):
    ...
    if criterion == 'loss':
        # loss should be -loglike, but since we are taking ratio it is ok
        ...
        print('Other criterion not yet supported')

    return result_df


def plot_perf_metric(perf_metric, eligible, x_range, 
                        select = None, plot_selected = False, x_breaks = None):
    ...
    plot = pn.ggplot(perf_df, pn.aes(x = 'perf_metric')) + \
        pn.geoms.geom_histogram(breaks = np.linspace(1, np.max(perf_metric)+0.01, 40)) + \
...
def find_u2(u2_min, u2_max, coef_optim, coef_optim_var, loss_func, 
                m, k, ratio_range, criterion="loss", epsilon=0.05, 
                random_state = 1234):
    ...
    df = mark_eligibility(
        coef_df=df, coef_optim=coef_optim, 
        loss_func=loss_func, criterion=criterion, epsilon=epsilon
    )
    ratio = np.sum(df['eligible']) / m

    if ratio >= max(ratio_range):
        # current u2 too small
        return find_u2(u2_min=u2, u2_max=u2_max, coef_optim=coef_optim, \
            coef_optim_var=coef_optim_var, loss_func=loss_func, m=m, k=k, \
                ratio_range=ratio_range, criterion=criterion, epsilon=epsilon, \
                    random_state=random_state)
    elif ratio <= min(ratio_range):
        # current u2 too large
        return find_u2(u2_min=u2_min, u2_max=u2, coef_optim=coef_optim, \
            coef_optim_var=coef_optim_var, loss_func=loss_func, m=m, k=k, \
                ratio_range=ratio_range, criterion=criterion, epsilon=epsilon, \
                    random_state=random_state)
    else:
